...
- **Table `public.notes`** with RLS (`user_id = auth.uid()`, same per-user isolation as `workouts`): `id, user_id, date, tags (text[]), text, created_at`. Tags include workout letter ("A"/"B") and/or exercise keys. Schema lives in `sql/create_notes_table.sql` (schema only — never commit actual note content); created in Supabase 2026-08-04. `fetchNotes()` also degrades gracefully to `[]` if the table is ever missing, so `finishWorkout()` can't false-fail on a notes-fetch error again.
